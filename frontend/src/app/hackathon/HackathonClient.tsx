 ...
 * PURPOSE: Interactive client-side interface for Learnova Hackathons.
...
const RULES_FAQ = [
  {
    q: "Who is eligible to participate in Learnova Hackathons?",
  ...
  },
  {
    ...
    a: "Teams can consist of 1 to 4 members. Team members must register individually on Learnova, then coordinate submissions using the Project Submission form."
  },
  {
...
export default function HackathonClient() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("learnova-registered-hackathons");
      if (stored) {
  ...
  const handleRegister = (id: string, title: string) => {
    ...
    localStorage.setItem("learnova-registered-hackathons", JSON.stringify(updated));
