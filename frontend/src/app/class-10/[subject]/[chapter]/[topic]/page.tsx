...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (subject !== "science" || !CLASS10_SCIENCE_CHAPTERS[chapter]) {
    return { title: "Topic Study | EduQuest" };
  }
  ...
  if (!topicData) {
    return { title: "Topic Not Found | EduQuest" };
  }

  const topicTitle = topicData.title.replace(/^\d+\.\s*/, ""); /* Strip "1. " prefix */

  return {
    title: `${topicTitle} — ${chapterData.title} | Class 10 Science | EduQuest`,
