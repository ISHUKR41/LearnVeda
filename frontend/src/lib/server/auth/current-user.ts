 ...
 *   LearnVeda user row is created just-in-time (JIT provisioning) so the student
...
function rowToPublicUser(row: LearnVedaUserRow): PublicUser {
  return {
    id:        row.id,
    name:      row.name,
    email:     row.email,
 ...
 * Looks up an LearnVeda user by email from Clerk data.
...
async function findOrCreateClerkUser(
  ...
  try {
    ...
    /* Try to find an existing LearnVeda account with this email */
    const existing = await pool.query<LearnVedaUserRow & { password_hash?: string }>(
    ...
    const inserted = await pool.query<LearnVedaUserRow>(
...
export async function getAuthenticatedUser(request: NextRequest): Promise<PublicUser | null> {
  ...
  try {
    ...
    if (userId) {
      ...
      try {
        const pool = getPostgresPool();
        const existing = await pool.query<LearnVedaUserRow>(
  ...
  try {
    ...
    const pool   = getPostgresPool();
    const result = await pool.query<LearnVedaUserRow>(
      `SELECT id, name, email, track, role, level, xp, streak, created_at
