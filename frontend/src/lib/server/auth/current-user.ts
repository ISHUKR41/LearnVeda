 ...
 *   VidyaBolt user row is created just-in-time (JIT provisioning) so the student
...
function rowToPublicUser(row: VidyaBoltUserRow): PublicUser {
  return {
    id:        row.id,
    name:      row.name,
    email:     row.email,
 ...
 * Looks up an VidyaBolt user by email from Clerk data.
...
async function findOrCreateClerkUser(
  ...
  try {
    ...
    /* Try to find an existing VidyaBolt account with this email */
    const existing = await pool.query<VidyaBoltUserRow & { password_hash?: string }>(
    ...
    const inserted = await pool.query<VidyaBoltUserRow>(
...
export async function getAuthenticatedUser(request: NextRequest): Promise<PublicUser | null> {
  ...
  try {
    ...
    if (userId) {
      ...
      try {
        const pool = getPostgresPool();
        const existing = await pool.query<VidyaBoltUserRow>(
  ...
  try {
    ...
    const pool   = getPostgresPool();
    const result = await pool.query<VidyaBoltUserRow>(
      `SELECT id, name, email, track, role, level, xp, streak, created_at
