...
function NavbarShell({ pathname }: { pathname: string }) {
  ...
  useEffect(() => {
    setMounted(true);
    const stored = localStorage.getItem("zingpath-theme");
    const prefersDark = stored ? stored === "dark" : true;
    setIsDark(prefersDark);
    if (!stored) localStorage.setItem("zingpath-theme", "dark");
  }, []);
  ...
  const toggleTheme = () => {
    ...
    setIsDark(newDark);
    localStorage.setItem("zingpath-theme", newDark ? "dark" : "light");
  };
    ...
    <nav className={`${styles.navbar} ${scrolled ? styles.navbarScrolled : ""}`} role="navigation">
      <div className={styles.navbarInner}>
        ...
        <Link href="/" className={styles.logo}>
          ...
          <span className={styles.logoText}>Zingpath</span>
        </Link>
